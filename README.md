...
[.github/workflows/daily.yml](.github/workflows/daily.yml) が毎日 4:00 JST に前日分だけを申請する
...
- **公開リポジトリ**なので Actions のログは誰でも読める。通常は要約行だけを出し、勤怠時刻や
  ...
  (Secrets の漏えい防止)。
- 稼働が 24 時をまたぐ日 (退勤 25:xx など) があるため 4:00 JST に実行している。それより遅く
  なる日があれば cron をさらに遅らせる。打刻済みの日は削除して入れ直すので再実行しても二重にならない。
- 失敗した日は `date` を指定した手動実行でやり直せる。既に申請中のものはスキップされる
  (打刻申請とスケジュール申請は別々に判定するので、片方だけ失敗した日もやり直せる)。
